...
const examples: Record<EditableMasterDataTable, string[]> = {
  ...
  fiscal_years: ["FY2026", "FY 2026", "ปีงบประมาณ 2569", "2026", "2025-10-01", "2026-09-30"],
  document_types: ["technical_specification", "Technical Specification", "ข้อกำหนดคุณลักษณะ"],
};
...
export async function buildMasterDataTemplate(
  ...
  const headers = table === "fiscal_years"
    ? [...baseHeaders, "year", "starts_on", "ends_on"]
    : baseHeaders;
  const importSheet = workbook.addWorksheet("Import", { views: [{ state: "frozen", ySplit: 1 }] });
  importSheet.columns = headers.map((header) => ({ header, key: header, width: Math.max(16, header.length + 5) }));
  importSheet.getRow(1).font = { bold: true, color: { argb: "FFFFFFFF" } };
  importSheet.getRow(1).fill = { type: "pattern", pattern: "solid", fgColor: { argb: "FF1559C7" } };
  importSheet.autoFilter = { from: { row: 1, column: 1 }, to: { row: 1, column: headers.length } };
  ...
  guide.addRows([
    [locale === "th" ? "แม่แบบนำเข้าข้อมูลหลัก" : "Master-data import template", ""],
    [locale === "th" ? "ภาษา" : "Language", language],
    [locale === "th" ? "วิธีใช้" : "How to use", locale === "th" ? "กรอกข้อมูลในแผ่นงาน Import เท่านั้น ห้ามเปลี่ยนชื่อหัวคอลัมน์ ลบแถวตัวอย่างนี้ได้" : "Enter records only in the Import sheet. Do not rename headers. The example below is not imported."],
