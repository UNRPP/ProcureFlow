...
| `SUPABASE_SERVICE_ROLE_KEY`     | Server only        | Cron only | Privileged key used solely by the scheduled reminder route. Never expose it to a Client Component. |
| `CRON_SECRET`                   | Server only        | Cron only | Shared bearer secret for `/api/cron/notifications`. Use at least 32 random bytes.                  |

Generate a cron secret locally with `openssl rand -base64 32`. Store production and preview values in the hosting provider's encrypted environment settings. Rotate `CRON_SECRET` and the service-role key immediately after suspected disclosure.
